...
class TransformedSubset(Dataset):
    """
    Wrap a Subset so that we can apply a transform on-the-fly,
    even if the underlying dataset has no transform.
    """
    ...
    def __getitem__(self, idx):
        x, y = self.subset[idx]
        # x is a PIL Image since train_set.transform=None
...
unlearning_dataset = TransformedSubset(unlearning_dataset, transform=train_transform)
...
multi_view_dataset_for_retrain = MultiViewMNIST(remaining_dataset, args.k, train_transform, train_transform)

remaining_loader = DataLoader(multi_view_dataset_for_retrain, batch_size=args.batch_size, shuffle=True)
